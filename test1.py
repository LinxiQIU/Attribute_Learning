...
# for p, l, t, A, n, m in train_dataloader:
    ...
    # print(dist)
    # bpos = np.array([x[10: 25] for x in attr_np])      # Size(16, 15)
    # # print(bpos.shape)
    # pred_bpos = np.array([x[10: 25] for x in pred_np])
    # m3 = np.array([x[i] for x in m_np for i in [10, 13, 16, 19, 22]])
    # # print(m3.shape)    # Size(16*5=80,)
    # dist3 = distance(bpos.reshape(-1), pred_bpos.reshape(-1), dim=3, mask=m3)
