...
class DGCNN_net(nn.Module):
    def __init__(self, output_channels=28):
        ...
        self.conv6 = nn.Sequential(nn.Conv1d(5, 64, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv7 = nn.Sequential(nn.Conv1d(3, 64, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
...
class DGCNN_cls(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dp1(F.leaky_relu(self.bn6(self.linear1(x)), negative_slope=0.2)) # (batch_size, emb_dims*2) -> (batch_size, 512)
        x = self.dp2(F.leaky_relu(self.bn7(self.linear2(x)), negative_slope=0.2)) # (batch_size, 512) -> (batch_size, 256)
