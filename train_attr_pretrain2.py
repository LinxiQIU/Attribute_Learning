...
from models import DGCNN_Core, Attribute
...
def train(args, io):
    ...
    if args.model_path is None:
        ...
        Head.load_state_dict(pretrained['model_state_dict'])
        print('Using pretrain head block!')
    ...
    for epoch in range(args.epochs):
        ...
        outstr = 'Train %d, Loss: %.6f, profile error: %.5f, gear pos mdist: %.5f, cbolt mdist: %.5f, motor rot: %.5f'%(epoch, 
            train_loss/count, train_profile_error, train_gpos_error, train_bpos_error, train_mrot_error)
        io.cprint(outstr)
        ...
        if val_loss/count <= best_mse:
            ...
            state = {'epoch': epoch, 'model_state_dict': Tail2.state_dict(), 'optimizer_state_dict': opt.state_dict()}
        ...
        outstr = 'Test %d, Loss: %.6f, profile error: %.5f, gear pos mdist: %.5f, cbolt mdist: %.5f, motor rot: %.5f' % (epoch, 
            val_loss*1.0/count, test_profile_error, test_gpos_error, test_bpos_error, test_mrot_error)
        io.cprint(outstr)
        io.cprint('\n\n')
